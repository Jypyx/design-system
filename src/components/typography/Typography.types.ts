 ...
 * label renders <label>, caption/overline render <span>.
 */
export type TypographyVariant =
  'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'subtitle' | 'body' | 'label' | 'caption' | 'overline'
...
export type TypographyTag =
  'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'p' | 'span' | 'div' | 'label' | 'legend' | 'figcaption'

export interface TypographyProps {
  variant?: TypographyVariant
  /**
   * Overrides the rendered tag while keeping the variant's style, e.g.
   * as="h2" variant="h5" to fit the document outline. The 'label' variant
   * renders a <label> by default (pass `for` as a normal attribute); use
   * as="span" when a <label> element doesn't fit the context (inside a
   * button or another <label>).
   */
  as?: TypographyTag
  /**
